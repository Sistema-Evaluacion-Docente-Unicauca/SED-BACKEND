...
/**
 * Servicio para gestionar ActividadVarchar con soporte para CRUD y paginación.
 */
@Service
@RequiredArgsConstructor
public class ActividadVarcharService {

    private final ActividadVarcharRepository actividadVarcharRepository;
    private static final Logger LOGGER = LoggerFactory.getLogger(ActividadVarcharService.class);
     ...
     */
    public ResponseEntity<ApiResponse<ActividadVarchar>> obtenerPorId(Integer id) {
        try {
            Optional<ActividadVarchar> actividadVarchar = actividadVarcharRepository.findById(id);
            return actividadVarchar.map(value -> ResponseEntity.ok(new ApiResponse<>(200, "Registro encontrado", value)))
